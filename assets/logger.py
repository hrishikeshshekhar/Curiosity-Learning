...
class Logger(object):
    ...
    def __init__(self, n_envs, logdir):
        ...
        self.episode_eval_rewards = deque(maxlen=40)
        ...
        self.log = pd.DataFrame(columns=['timesteps', 'wall_time', 'num_episodes',
                                         ...
                                         'max_episode_len', 'mean_episode_len', 'min_episode_len', 'mean_eval_rewards', 'max_eval_reward'])
        self.writer = SummaryWriter(logdir)
        self.timesteps = 0
        self.num_episodes = 0

    def feed(self, rew_batch, done_batch, eval_batch, eval_max):
        steps = rew_batch.shape[0]
        rew_batch = rew_batch.T
        done_batch = done_batch.T
        self.episode_eval_rewards = eval_batch
    ...
    def _get_episode_statistics(self):
        ...
        episode_statistics['Rewards/mean_eval_rewards'] = np.mean(self.episode_eval_rewards)
        episode_statistics['Rewards/max_eval_reward'] = np.array(self.eval_max)
