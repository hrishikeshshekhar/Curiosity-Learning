    ...
    def fetch_train_generator(self, mini_batch_size=None, recurrent=False):
        ...
        if not recurrent:
            ...
            for indices in sampler:
                ...
                rew_batch = torch.FloatTensor(self.rew_batch).reshape(-1)[indices].to(self.device)   #changed
                yield obs_batch, hidden_state_batch, act_batch, done_batch, log_prob_act_batch, value_batch, return_batch, adv_batch, rew_batch  #changed
        # If agent's policy is recurrent, data should be sampled along the time-horizon
            ...
            for start_ind in range(0, self.num_envs, num_envs_per_batch):
                ...
                rew_batch = torch.FloatTensor(self.rew_batch[:, idxes]).reshape(-1).to(self.device)  #changed
                yield obs_batch, hidden_state_batch, act_batch, done_batch, log_prob_act_batch, value_batch, return_batch, adv_batch, rew_batch #changed
